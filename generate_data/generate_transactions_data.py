...
non_expiring_product_ids = dict(
    zip(non_expiring_products["name"], non_expiring_products["id"])
)
...
expiring_count = transaction_df[
    transaction_df["product_name"].isin(expiring_product_ids.keys())
].shape[0]
non_expiring_count = transaction_df[
    transaction_df["product_name"].isin(non_expiring_product_ids.keys())
].shape[0]

print(f"Expiring products transactions: {expiring_count}")
print(f"Non-expiring products transactions: {non_expiring_count}")
print(
    f"Percentage of expiring products: {expiring_count / len(transactions) * 100:.2f}%"
)
