...
def get_expiry_days(__date):
    return (datetime.datetime.strptime(__date, "%Y-%m-%d") - datetime.datetime.now()).days


PRODUCT_DATA = pd.read_csv("data/product_data_with_carbon_footprint.csv")
PRODUCT_DATA.expiresAt = PRODUCT_DATA.expiresAt.apply(get_expiry_days)
PRODUCT_DATA_DICT = dict(
    zip(
        PRODUCT_DATA["id"],
        PRODUCT_DATA[
            ["id", "name", "price", "carbon_footprint", "expiresAt"]
        ].values.tolist(),
    )
)
...
def recommend_products(customer_index, num_recommendations=20):
    ...
    recommendations = MODEL.recommend(
        customer_index, user_interaction, N=num_recommendations
    )
...
def get_all_recos(customer_index):
    ...
    personalized_recos_with_product_data = [
        get_product_data(__id) for __id in personalized_recos
    ]
    product_bundle_with_product_data = [
        get_product_data(product_info[0]) for product_info in product_bundle
    ]

    personalized_recos_with_product_data = sorted(
        personalized_recos_with_product_data,
        key=lambda x: x[-1],
    )
    return {
        "personalized_recos": personalized_recos_with_product_data,
